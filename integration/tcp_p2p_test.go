...
func TestP2PTunnelUsesDirectPathWithRendezvous(t *testing.T) {
...
func TestP2PTunnelFallsBackWhenSetupFails(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	store, authService := testServices(t)
...
func TestTCPTunnelDoesNotAttemptP2P(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	store, authService := testServices(t)
	...
	assertNoEventRecorded(t, store, "p2p.tcp.fallback")
}
