...
export function TunnelsPage() {
  ...
  return (
    <Page title="Tunnels" loading={state.loading} error={state.error} empty={!tunnels.length} onReload={state.reload}
      ...
      <Table<Tunnel>
        ...
        columns={[
          ...
          { title: "Local", render: (_, t) => `${t.local_host}:${t.local_port}` },
          { title: "Remote", render: (_, t) => t.domain || t.remote_port || "-" },
...
function TunnelEditor(props: { tunnel: Tunnel | null; clients: Client[]; onCancel: () => void; onSave: (v: TunnelForm) => void }) {
  ...
  return (
    <Modal title={props.tunnel?.id ? "编辑 Tunnel" : "创建 Tunnel"} open={!!props.tunnel} onCancel={props.onCancel} onOk={() => document.getElementById("tunnel-save")?.click()} destroyOnClose width={720}>
      <Form layout="vertical" initialValues={initial} onFinish={props.onSave}>
        <div className="form-grid">
          <Form.Item name="name" label="名称" rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="client_id" label="Client" rules={[{ required: true }]}><Select options={props.clients.map((c) => ({ value: c.id, label: c.name }))} /></Form.Item>
          ...
          <Form.Item name="status" label="状态"><Select options={["stopped", "running"].map((v) => ({ value: v, label: v }))} /></Form.Item>
