    ...
    <div className="sparkline" aria-label="实时上传流量趋势">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 8, right: 0, bottom: 0, left: 0 }}>
          <Area type="monotone" dataKey="upload" stroke="#12ad67" strokeWidth={2} fill="#e3f7ec" fillOpacity={0.9} isAnimationActive={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
    ...
    <div className="traffic-chart" aria-label="实时上传和下载流量趋势">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 12, right: 12, bottom: 0, left: -18 }}>
          ...
          <Line type="monotone" dataKey="upload" name="上传 KB/s" stroke="#12ad67" strokeWidth={2} dot={false} isAnimationActive={false} />
          <Line type="monotone" dataKey="download" name="下载 KB/s" stroke="#2f7df6" strokeWidth={2} dot={false} isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
