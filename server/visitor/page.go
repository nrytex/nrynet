...
<script>
const config=__NRYNET_CONFIG__,state={pending:new Map(),sendQueue:Promise.resolve(),sequence:0};let peer,channel;
...
function sendFrame(value){const task=state.sendQueue.catch(()=>{}).then(()=>{if(!channel||channel.readyState!=='open')throw new Error('P2P channel is not open');channel.send(JSON.stringify(value))});state.sendQueue=task.catch(()=>{});return task}
...
async function forwardFetch(message,port){const id=String(++state.sequence),body=message.body||new ArrayBuffer(0);state.pending.set(id,{port});try{await sendFrame({kind:'request_start',id,method:message.method,path:message.path,headers:message.headers||{}});const bytes=new Uint8Array(body);for(let offset=0;offset<bytes.length;offset+=24576)await sendFrame({kind:'request_chunk',id,body:toBase64(bytes.subarray(offset,Math.min(offset+24576,bytes.length)))});await sendFrame({kind:'request_end',id})}catch(error){state.pending.delete(id);port.postMessage({kind:'response_end',id,error:error.message||String(error)})}}
async function connect(){
 peer=new RTCPeerConnection({iceServers:(config.ice_servers||[]).map(urls=>({urls}))});channel=peer.createDataChannel('nrynet-visitor');channel.onmessage=handleFrame;
 channel.onopen=()=>{setStatus('P2P connected to Agent','ok');frame.src=config.scope_url};channel.onclose=()=>{setStatus('P2P connection closed','err');frame.removeAttribute('src')};channel.onerror=()=>setStatus('P2P data channel error','err');
